...
headers = {
    ...
    'Cookie': '''SUB=_2A25NDrosDeRhGeNH7VsV-SrKyDWIHXVu8MZkrDV6PUJbkdANLWLWkW1NSoaARVwG1WKGlTMa36lVNu1KQkpVrNrx; SCF=AsFKLKPD2RMOLYYn1jxcshLADRbDtr0ONTyUmtLHgGIsQpTrI1OB3VDtIJDTWI2JGPsj1anJBagDdaOyRCt5NLE.; SSOLoginState=1611319932; _T_WM=26323701299; WEIBOCN_FROM=1110006030; MLOGIN=1; M_WEIBOCN_PARAMS=uicode%3D20000174'''
}
...
class WeiboCommentScrapy(Thread):
    ...
    def write_to_csv(self, result, isHeader=False):
        with open('comment/' + 'comment_yangshixinwen.csv', 'a', encoding='utf-8-sig', newline='') as f:
            writer = csv.writer(f)
            if isHeader == True:
                writer.writerows([self.result_headers])
            writer.writerows(result)
        print('已成功将{}条评论写入{}中'.format(len(result), 'comment/' + 'comment_yangshixinwen.csv'))
...
if __name__ == "__main__":
    weibo_list = []
    document = open("yangshixinwen.json", encoding='utf-8')
