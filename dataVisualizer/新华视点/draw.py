...
#标题
line_chart.title = '不同时间下新华社的微博与疫情相关度的变化'
#X轴数据
...
line_chart.render_to_file('不同时间下新华社的微博与疫情的相关度变化.svg')

filename = '1699432410.json'
with open(filename, encoding='utf-8') as f:
    btc_data = json.load(f)
...
#标题
line_chart.title = '不同点赞数的新华社微博的条数'
#X轴数据
...
line_chart.render_to_file('不同点赞数的新华社微博的条数.svg')
...
#标题
line_chart.title = '不同转发数的新华社微博的条数'
#X轴数据
line_chart.x_labels = list(b.keys())
#X轴坐标隔20天显示一次
N = 800
...
#添加Y轴信息
line_chart.add('条数', list(b.values()))
line_chart.render_to_file('不同转发数的新华社微博的条数.svg')
...
#标题
line_chart.title = '不同评论数的新华社微博的条数'
#X轴数据
line_chart.x_labels = list(b.keys())
#X轴坐标隔20天显示一次
N = 800
...
#添加Y轴信息
line_chart.add('条数', list(b.values()))
line_chart.render_to_file('不同评论数的新华社微博的条数.svg')